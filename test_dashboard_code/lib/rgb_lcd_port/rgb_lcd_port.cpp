...
esp_lcd_panel_handle_t waveshare_esp32_s3_rgb_lcd_init()
{
    ...
    esp_lcd_rgb_panel_config_t panel_config = {
        ...
        .timings = {
            ...
            /* Timings d'origine (image stable) */
        ...
        .num_fbs = EXAMPLE_LCD_RGB_BUFFER_NUMS,                  // 3 framebuffers
    ...
    ESP_LOGI(TAG, "Initialize RGB LCD panel (pclk=%d Hz, fb=%d)", EXAMPLE_LCD_PIXEL_CLOCK_HZ, EXAMPLE_LCD_RGB_BUFFER_NUMS);
...
// /*****************************************************************************
//  * rgb_lcd_port.cpp - Driver RGB panneau 1024x600
//  ******************************************************************************/

//  /*****************************************************************************
//  * rgb_lcd_port.cpp - Baseline Arduino direct mode
//  ******************************************************************************/
// #include "rgb_lcd_port.h"
// #include "lvgl_port.h"
...
//     ESP_ERROR_CHECK(esp_lcd_new_rgb_panel(&panel_config, &panel_handle));
//     ESP_LOGI(TAG, "Initialize panel (PCLK=%u Hz)", EXAMPLE_LCD_PIXEL_CLOCK_HZ);
//     ESP_ERROR_CHECK(esp_lcd_panel_init(panel_handle));
...
//     uint8_t *dst = (uint8_t *)malloc(w * h * 2);
//     if (!dst) return;

//     for (int y = 0; y < h; y++) {
//         const uint8_t *src_row = Image + ((Ystart + y) * EXAMPLE_LCD_H_RES + Xstart) * 2;
//         memcpy(dst + y * w * 2, src_row, w * 2);
//     }
//     esp_lcd_panel_draw_bitmap(panel_handle, Xstart, Ystart, Xend, Yend, dst);
//     free(dst);
// }
...
//                               EXAMPLE_LCD_H_RES, EXAMPLE_LCD_V_RES, Image);
// }
